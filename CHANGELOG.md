...
### Added
- **Exact text search:** In the search modal, use `"phrase"`, repeated quotes or `+word` for literal AND, `/regex/`, or a leading `~` for case-sensitive matching. A live badge shows the active mode; semantic embedding is skipped for these queries. Settings → Relevance documents syntax and the Name → Exact text footer stage.
...
### Fixed
- **Hovering a recent search then pressing Enter** replays that query (same as clicking the row or ↑/↓ then Enter). Hover previously only restyled the row without selecting it, so Enter did nothing while the field was empty.
- **Search modal hotkeys no longer register globally.** Default chords (Mod+Enter, Alt+Enter, Mod+Shift+E, Tab, ↑/↓, etc.) apply only while the Seek search modal is focused — via the query field and a modal capture listener — so they do not hijack the editor or other Obsidian UI when the modal is closed. Remap under Settings → Hotkeys; bindings still apply while the modal has focus. Insert-link chords additionally require an active markdown editor.
...
- **Settings → Index → Embedder coverage by folder** is a live indexed/total snapshot: overall and each folder show the same `covered / total` fraction and a **two-decimal** percent (`99.57%`, never a rounded-up 100% while files remain). Folder bars stay yellow while that folder still has remaining or in-flight notes, including when several folders need work at once. Vault-root notes appear as their own row so the headline cannot read 100% while a component does not. Hover the % or count for remaining / in-flight detail. Excluded folders still show an em dash. Refreshes from indexing events (dirty queue, computeDelta, committed bursts); polls every 1s while work remains, 5s when idle. Expanding a folder re-renders the last cached tree (no IDB wait).
...
- **Settings “Download now” and model status after delete/unload.** Deleting the embedding model or tearing down the runtime left a stale resolved load memo, so prewarm/search returned instantly without re-fetching and the settings row stayed on “Not downloaded”. `releaseEmbedder()` now clears the memo; cache probing accepts URL-encoded ONNX paths.
