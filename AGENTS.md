...
- **Line endings must be LF.** The atom parser (`src/atoms.ts`) and token-budget/chunking code split on `\n`; a CRLF checkout leaves stray `\r` that breaks fence detection and fails the `atoms` and `token-budget` suites (13 failures). `.gitattributes` enforces `eol=lf`. On Windows, if you cloned before that existed, run `git config core.autocrlf input` then `git rm --cached -r . && git reset --hard` (or `git add --renormalize .`) to convert the working tree to LF.
