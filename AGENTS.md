...
- **Finish gate (cloud sessions):** Obsidian Plugin Development skill `obsidian-cloud-vm-demos` — VM runtime proof + mandatory screenshots for visible changes; local sandbox does not replace cloud proof. When a PR exists, embed demo screenshots/video in the **PR description** (Cursor dashboard: **Allow posting artifacts to GitHub**; see [Artifacts in GitHub](https://cursor.com/docs/cloud-agent/capabilities#artifacts-in-github)).
...
- **Architectural / major method changes → check tests with a subagent.** When a change rewires call order, moves fields onto new owners (e.g. schedulers), or adds hooks inside hot paths like `runCatchUp` / `drainCatchUp`, launch an **explore** subagent to find thin harnesses and prototype-based tests that stub only the old surface (e.g. `Object.create(SeekPlugin.prototype)`). Update those stubs in the same change; otherwise `npm test` can fail later with misleading “behavior broken” symptoms when the fake is simply incomplete.
...
3. `obsidian plugin:enable id=seek vault=plugin-sandbox-Obsidian` — **do not reload**. If enable fails or IDB delete was `blocked`, use global restart only after confirming the user accepts closing **all** open vaults (see plugin skill `obsidian-multi-vault-cli`).
...
User-facing features, fixes, and settings changes: add bullets to `CHANGELOG.md` `[Unreleased]` when ready; version bump on `main` only — see plugin skill `obsidian-plugin-dev` (Obsidian Plugin Development) — Release notes and semantic versioning. Upstream sync adopts upstream version; fork-only patches accumulate under `[Unreleased]` until a fork release on `main`.
