...
- **Startup path worktrees:** Trace infra on `startup/trace-infra`; isolated path branches (`path/greedy-hydrate`, etc.) measure via `startup-trace-probe.ps1` → `gate-trace.jsonl` + `parse-startup-trace.mjs` → `.cursor/scorecards/` and `.cursor/startup-path-results.md`.
