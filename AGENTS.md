...
3. `obsidian plugin:enable id=seek vault=plugin-sandbox-Obsidian` — **do not reload**. If enable fails or IDB delete was `blocked`, use `obsidian restart vault=plugin-sandbox-Obsidian` only after confirming the user accepts closing **all** open vaults (see `.cursor/rules/obsidian-multi-vault-cli.mdc`).
