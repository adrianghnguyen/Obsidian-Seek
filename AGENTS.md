...
Copy `main.js`, `manifest.json`, and `styles.css` to the sandbox plugin path for testing. Do **not** copy to `C:\Obsidian\.obsidian\plugins\seek\` during feature branches or routine task completion. Policy: `.cursor/rules/seek-vault-cli.mdc`.
...
3. `obsidian plugin:enable id=seek vault=plugin-sandbox-Obsidian` — **do not reload**. If enable fails or IDB delete was `blocked`, use global restart only after confirming the user accepts closing **all** open vaults (see `.cursor/rules/seek-vault-cli.mdc`).
